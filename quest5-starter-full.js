...
removeSceneObject(object){const index=this._objects.indexOf(object);if(index!==-1){this._objects.splice(index,1);}}
...
updateGeometry(){}
loadShader(filename){console.log(filename);return new Promise((resolve,reject)=>{const xhttp=new XMLHttpRequest();xhttp.open("GET",filename);xhttp.setRequestHeader("Cache-Control","no-cache, no-store, max-age=0");xhttp.onload=function(){if(xhttp.readyState===XMLHttpRequest.DONE&&xhttp.status===200){resolve(xhttp.responseText);}
...
class PolygonObject extends SceneObject{constructor(device,canvasFormat,filename){super(device,canvasFormat);this._polygon=new Polygon(filename);this._device=device;this._canvasFormat=canvasFormat;this._currentState=0;this._stiffness=1.0;this._color=[0.0,1.0,0.0,1.0];this._gravityEnabled=false;}
...
async createShaders(){const computeShaderCode=await this.loadShader("/shaders/optimized_arap.wgsl");this._arapShaderModule=this._device.createShaderModule({label:"ARAP Shader "+this.getName(),code:computeShaderCode,});const renderShaderCode=await this.loadShader("/shaders/optimized_standard2d.wgsl");this._renderShaderModule=this._device.createShaderModule({label:"Render Shader "+this.getName(),code:renderShaderCode,});this._pipelines={gravity:this._device.createComputePipeline({layout:'auto',compute:{module:this._arapShaderModule,entryPoint:'gravityUpdate'}}),};}
dispatchGravityUpdate(encoder){if(!this._gravityEnabled)return;const floorY=-0.9;this._device.queue.writeBuffer(this._stiffnessBuffer,0,new Float32Array([this._stiffness]));const bindGroup=this._device.createBindGroup({layout:this._pipelines.gravity.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this._stateBuffers[this._currentState]}},{binding:1,resource:{buffer:this._stateBuffers[1-this._currentState]}},{binding:2,resource:{buffer:this._stiffnessBuffer}},]});const pass=encoder.beginComputePass();pass.setPipeline(this._pipelines.gravity);pass.setBindGroup(0,bindGroup);const workgroupSize=256;const numGroups=Math.ceil(this._vertexCount/workgroupSize);pass.dispatchWorkgroups(numGroups);pass.end();}
...
set gravityEnabled(flag){this._gravityEnabled=flag;}
get gravityEnabled(){return this._gravityEnabled;}
...
return winding!==0;}
async function switchPolygon(){if(currentModelIndex>=4){currentModelIndex=0;}else{currentModelIndex+=1;}
const file=polygonFiles[currentModelIndex];console.log("Switching polygon to:",file);console.log(currentModelIndex);renderer.removeSceneObject(polygon);polygon=new PolygonObject(renderer._device,renderer._canvasFormat,file);await renderer.appendSceneObject(polygon);}
window.addEventListener('keydown',(e)=>{if(e.key=='g'){polygon.gravityEnabled=!polygon.gravityEnabled;console.log('Gravity toggled:',polygon.gravityEnabled);polygon.dispatchGravityUpdate(encoder);}
if(e.key==='ArrowUp'){console.log("UP")
polygon._stiffness=Math.min(polygon._stiffness+0.1,5.0);console.log('Stiffness increased:',polygon._stiffness);}else if(e.key==='s'||e.key==='S'){console.log("Switching polygon...");switchPolygon();}else if(e.key==='ArrowDown'){console.log("DOWN")
...
return closest;}
init().then(ret=>{console.log(ret);const controlsBox=document.createElement('div');controlsBox.style.position='absolute';controlsBox.style.top='10px';controlsBox.style.right='10px';controlsBox.style.padding='12px';controlsBox.style.backgroundColor='rgba(0, 0, 0, 0.7)';controlsBox.style.color='white';controlsBox.style.fontFamily='monospace';controlsBox.style.fontSize='14px';controlsBox.style.borderRadius='8px';controlsBox.style.zIndex='1000';controlsBox.style.maxWidth='300px';controlsBox.innerHTML=`
  <b>Controls</b><br/>
  G - Toggle gravity<br/>
  R - Reset simulation<br/>
  ↑ / ↓ - Adjust stiffness<br/>
  S - Switch polygon model<br/>
  Mouse - Move to polygon for inside dection<br/>
`;document.body.appendChild(controlsBox);}).catch(error=>{const pTag=document.createElement('p');pTag.innerHTML=navigator.userAgent+"</br>"+error.message;document.body.appendChild(pTag);document.getElementById("renderCanvas").remove();});