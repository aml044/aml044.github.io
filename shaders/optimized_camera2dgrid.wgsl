...
fn vertexMain(@location(0) pos: vec2f, @builtin(instance_index) idx: u32) -> VertexOutput {
  let u = idx % GRID_SIZE; // we are expecting 10x10, so modulo 10 to get the x index
  let v = idx / GRID_SIZE; // divide by 10 to get the y index
  let uv = vec2f(f32(u), f32(v)) / f32(GRID_SIZE); // normalize the coordinates to [0, 1]
  let halfLength = 1.f; // half cell length
  let cellLength = halfLength * 2.f; // full cell length
  let cell = pos / f32(GRID_SIZE); // divide the input quad into 10x10 pieces
  let offset = -halfLength + uv * cellLength + cellLength / f32(GRID_SIZE); // GRID_SIZE * 0.5; // compute the offset for the instance
  // Apply motor
...
fn computeMain(@builtin(global_invocation_id) cell: vec3u) {
  ...
  let x = cell.x;
  let y = cell.y;
  let neighborsAlive = cellStatusIn[(y) * GRID_SIZE + (x + 1)] + cellStatusIn[(y) * GRID_SIZE + (x - 1)] +
                       cellStatusIn[(y + 1) * GRID_SIZE + (x)] + cellStatusIn[(y - 1) * GRID_SIZE + (x)];
  let i = y * GRID_SIZE + x;
  // Compute new status  
