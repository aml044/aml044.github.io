...
@group(0) @binding(1) var<uniform> isLaplacian: u32;
@group(0) @binding(2) var<uniform> renderConfig: RenderConfig;
...
fn vertexMain(@location(0) srcpos: vec3f, @location(1) srcnormal: vec3f, @location(2) tgtpos: vec3f, @location(3) tgtnormal: vec3f) -> VertexOut {
  ...
  if (isLaplacian == 0){
    var interpolatePt = mix(srcpos, tgtpos, cameraPose.t);
    ...
    out.normal = normalize(mix(srcnormal, tgtnormal, cameraPose.t));
    return out;
  }
  else{
    var interpolatePt = srcpos;
    // transform and project 
    var transformedPt = applyMotorToPoint(interpolatePt, reverse(cameraPose.motor));
    var projectedPt = vec2f(transformedPt.x/transformedPt.z, transformedPt.y/transformedPt.z) * cameraPose.focal;
    // compute Proper Depth for WebGPU Clip Space
...
fn fragmentMain(@builtin(position) fragCoord: vec4f) -> @location(0) vec4f {
  ...
  let rayOrigin = getRayOrigin();   // however you compute ray origin
  let rayDir = getRayDirection();   // however you compute ray direction

  var color = vec4f(0.0, 0.0, 0.0, 1.0);
  var hits = rayVolumeIntersection(rayOrigin, rayDir);
  let epsilon = 1e-4;
  ...
  if (hits.x >= 0) {
    ...
    renderConfig.renderMode = 0u;

    if (renderConfig.renderMode == 1u) {
      // --- DRR ---
      var attenuation = 0.0;
      while (t < hits.y) {
        let pt = rayOrigin + rayDir * t;
        ...
        if (idx >= 0 && idx < i32(arrayLength(&volData))) {
          let val = volData[idx];
          let vmin = idx3D * voxelsize - vec3f(0.5);
          let vmax = vmin + voxelsize;
          var nextT = hits.y;
          nextT = getNextHitValue(hits.x, nextT, vmin.z, vmin.xy, vmax.xy, rayOrigin.z, rayDir.z, rayOrigin.xy, rayDir.xy);
          nextT = getNextHitValue(hits.x, nextT, vmax.z, vmin.xy, vmax.xy, rayOrigin.z, rayDir.z, rayOrigin.xy, rayDir.xy);
          nextT = getNextHitValue(hits.x, nextT, vmin.x, vmin.yz, vmax.yz, rayOrigin.x, rayDir.x, rayOrigin.yz, rayDir.yz);
          nextT = getNextHitValue(hits.x, nextT, vmax.x, vmin.yz, vmax.yz, rayOrigin.x, rayDir.x, rayOrigin.yz, rayDir.yz);
          nextT = getNextHitValue(hits.x, nextT, vmin.y, vmin.xz, vmax.xz, rayOrigin.y, rayDir.y, rayOrigin.xz, rayDir.xz);
          nextT = getNextHitValue(hits.x, nextT, vmax.y, vmin.xz, vmax.xz, rayOrigin.y, rayDir.y, rayOrigin.xz, rayDir.xz);
    ...
    } else if (renderConfig.renderMode == 2u) {
      // --- DEPTH ENCODING ---
    ...
    } else if (renderConfig.renderMode == 0u) {
      // --- MIP ---
...
fn fragmentMain(@location(0) normal: vec3f) -> @location(0) vec4f {
  ...
  if (renderConfig.renderMode == 2u){
      return vec4f((normal + 2) * 0.5, 1); // simple normal color
  } else if (renderConfig.renderMode == 1u){
        return vec4f((normal + 1) * 0.5, 1); // simple normal color
  } else {
        return vec4f((normal) * 0.5, 1); // simple normal color
  }
}
