...
@group(0) @binding(4) var<uniform> renderMode: u32;
@group(0) @binding(5) var<uniform> lightMode: u32; 
@group(0) @binding(6) var sampDiffuse: sampler;
@group(0) @binding(7) var texDiffuse: texture_2d<f32>;
@group(0) @binding(8) var<uniform> useTexture: i32;
@group(0) @binding(9) var sampNormal: sampler;
@group(0) @binding(10) var texNormal: texture_2d<f32>;
@group(0) @binding(11) var sampEnv: sampler;
@group(0) @binding(12) var texEnv: texture_2d<f32>;
...
fn getLightInfo(lightPos: vec3f, lightDir: vec3f, hitPoint: vec3f, objectNormal: vec3f) -> LightInfo {
  ...
  if(lightMode == 0u){
    //point Light
      ...
      intensity /= factor;
      finalDir = normalize(hitPoint - lightPos);
  } else if (lightMode == 1u){
    //Directional Light
    finalDir = normalize(-lightDir);
  } else if (lightMode == 2u){
    // Spotlight 
    let lightToPoint = normalize(hitPoint - lightPos);
    let cosAngle = dot(lightToPoint, normalize(-lightDir));
    let cutoff = light.params.x; 
    if (cosAngle < cutoff) {
      let cutoff = light.params.x;   // e.g. cos(inner angle)
      let outerCutoff = light.params.y; // e.g. cos(outer angle)
      let epsilon = outerCutoff - cutoff;
      let intensityFactor = clamp((cosAngle - cutoff) / epsilon, 0.0, 1.0);
    ...
    finalDir = lightToPoint;
  }

  var out: LightInfo;
  out.intensity = intensity * max(dot(finalDir, -objectNormal), 0.0);
  out.lightdir = finalDir;
  return out;
}
...
fn computeOrthogonalMain(@builtin(global_invocation_id) global_id: vec3u) {
  ...
  if (uv.x < texDim.x && uv.y < texDim.y) {
    ...
    if (hitInfo.x > 0) { // if there is a hit
      ...
      //   7. finally, modulate the color by the light
      
            // transform incoming vectors
      normal = normalize(normal);
      let viewDir = normalize(-rdir); // camera ray direction
      let reflectDir = reflect(-lightInfo.lightdir, normal);

      var baseColor = diffuse.rgb;
      if(useTexture == 1) {
        let texUV = hitPt.xy * 0.5 + vec2f(0.5);
        baseColor = textureSampleLevel(texDiffuse, sampDiffuse, texUV, f32(0)).rgb;
      }

      diffuse *= vec4f(baseColor, 1.0);

      // material properties (you can tune them later)
      let ks = vec3f(1.0, 1.0, 1.0);  // specular highlight color (white)
      let ka = vec3f(0.1, 0.1, 0.1);  // ambient color (low background light)
      let shininess = 50.0;           // how shiny the specular spot is
      ...
      if (renderMode == 0) {
        ...
        let specAngle = max(dot(normalize(-rdir), normalize(R)), 0.0);
        let specular = (ks * pow(specAngle, shininess)) * lightInfo.intensity.xyz;

        let ambient = ka * lightInfo.intensity.xyz;
        color = vec4f(emit.rgb + diffuse.rgb + specular + ambient, 1.0);
      } 
        ...
        // Quantize final color into bands
        color = vec4f(
          quantize(color.r),
          quantize(color.g),
          quantize(color.b),
...
fn computeProjectiveMain(@builtin(global_invocation_id) global_id: vec3u) {
  ...
  let texDim = vec2i(textureDimensions(outTexture));
  if (uv.x < texDim.x && uv.y < texDim.y){
    // Compute the pixel position in normalized device coordinates
    let px = (f32(uv.x) / cameraPose.res.x - 0.5) * 2.0;
    let py = (0.5 - f32(uv.y) / cameraPose.res.y) * 2.0;

    // Projective camera: origin at (0,0,0), rays go through (px * focal.x, py * focal.y, 1)
    ...
    // Trace ray against box
    var hitInfo = rayBoxIntersection(spt, rdir);

    // Default background color
    var color = vec4f(0.f/255, 56.f/255, 101.f/255, 1.); // Bucknell Blue

    if (hitInfo.x > 0) { // if we hit the box
      let emit = boxEmitColor();
      ...
      normal = normalize(normal);
      let viewDir = normalize(-rdir);
      let reflectDir = reflect(-lightInfo.lightdir, normal);

      let ks = vec3f(1.0, 1.0, 1.0);
      let ka = vec3f(0.1, 0.1, 0.1);
      let shininess = 50.0;
      ...
      if (renderMode == 0) {
        ...
        // Phong
        let R = reflect(-lightInfo.lightdir, normal);
        let specAngle = max(dot(viewDir, normalize(R)), 0.0);
        let specular = (ks * pow(specAngle, shininess)) * lightInfo.intensity.xyz;
        let ambient = ka * lightInfo.intensity.xyz;
        diffuse *= lightInfo.intensity;
        color = vec4f(emit.rgb + diffuse.rgb + specular + ambient, 1.0);
