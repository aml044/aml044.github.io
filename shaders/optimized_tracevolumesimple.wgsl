...
//Sturct to change render mode
struct RenderConfig {
  renderMode: u32,  // 0 = MIP, 1 = DRR
}
...
@group(0) @binding(4) var<uniform> renderConfig: RenderConfig;
...
fn cinematicTF(val: f32) -> vec4f {
  let v = clamp(val / 4095.0, 0.0, 1.0);
  return vec4f(pow(v, 2.2), pow(v, 1.6), v, 1.0); // bluish cinematic tone
}

fn linearTF(val: f32) -> vec4f {
  let v = clamp(val / 4095.0, 0.0, 1.0);
  return vec4f(v, v, v, 1.0); // standard grayscale
}

fn spectralEdgeTF(val: f32) -> vec4f {
  let v = clamp(val / 4095.0, 0.0, 1.0);
  return vec4f(v, 1.0 - v, sin(v * 6.283), 1.0); // pseudo-spectral mapping
}

// a function to trace the volume - volume rendering
fn traceScene(uv: vec2i, p: vec3f, d: vec3f) {
  // textureStore(outTexture, uv, vec4f(1.0, 0.0, 0.0, 1.0));
  // return;

  var hits = rayVolumeIntersection(p, d);
  /*
  if (hits.x < 0.0 || hits.y < 0.0) {
    textureStore(outTexture, uv, vec4f(0.0, 0.0, 1.0, 1.0)); 
    return;
  }
  */
  ...
  var debugColor = vec4f(hits.x / 10.0, hits.y / 10.0, 0.0, 1.0);
  //textureStore(outTexture, uv, debugColor);
  //return;
    
  if (hits.x >= 0) {
    let voxelsize = volInfo.sizes.xyz / max(max(volInfo.dims.x, volInfo.dims.y), volInfo.dims.z);
    var t = hits.x + epsilon;
    let useMIP = (renderConfig.renderMode == 0u);
    let useDRR = (renderConfig.renderMode == 1u);
    let useDepthEncoding = (renderConfig.renderMode == 2u);
    let useCinematic = (renderConfig.renderMode == 3u);
    let useLinear = (renderConfig.renderMode == 4u);
    let useSpectral = (renderConfig.renderMode == 5u);
    ...
    if (useDRR){
      ...
      while(t < hits.y){
        let pt = p + d * t;
        let idx3D = floor((pt + 0.5) / voxelsize);
        let idx = i32(idx3D.z) * i32(volInfo.dims.x * volInfo.dims.y) +
                  i32(idx3D.y) * i32(volInfo.dims.x) +
                  i32(idx3D.x);
        if (idx >= 0 && idx < i32(arrayLength(&volData))){
          let val = volData[idx];

          //voxel corners
          let vmin = idx3D * voxelsize - vec3f(0.5, 0.5, 0.5);
          let vmax = vmin + voxelsize;

          var nextT = hits.y;
          nextT = getNextHitValue(hits.x, nextT, vmin.z, vmin.xy, vmax.xy, p.z, d.z, p.xy, d.xy);
          nextT = getNextHitValue(hits.x, nextT, vmax.z, vmin.xy, vmax.xy, p.z, d.z, p.xy, d.xy);
          nextT = getNextHitValue(hits.x, nextT, vmin.x, vmin.yz, vmax.yz, p.x, d.x, p.yz, d.yz);
          nextT = getNextHitValue(hits.x, nextT, vmax.x, vmin.yz, vmax.yz, p.x, d.x, p.yz, d.yz);
          nextT = getNextHitValue(hits.x, nextT, vmin.y, vmin.xz, vmax.xz, p.y, d.y, p.xz, d.xz);
          nextT = getNextHitValue(hits.x, nextT, vmax.y, vmin.xz, vmax.xz, p.y, d.y, p.xz, d.xz);

          let deltaT = nextT - t + epsilon;
          attenuation += deltaT * val / 4095.0;
          t = nextT + epsilon;
        }
        else {
          break;
      ...
      let intensity = clamp(1.0 - exp(-attenuation), 0.0, 1.0);
      color = vec4f(intensity, intensity, intensity, 1.0);
    } else if (useDepthEncoding){
      //depthEncoding
      var accumColor = vec4f(0, 0, 0, 1);
      while (t < hits.y){
        let pt = p + d * t;
        let idx3D = floor((pt + 0.5) / voxelsize);
                let idx = i32(idx3D.z) * i32(volInfo.dims.x * volInfo.dims.y) +
        ...
        if (idx >= 0 && idx < i32(arrayLength(&volData))){
          ...
          let deltaT = nextT - t + epsilon;
          accumColor += deltaT * val / 4095.0 * depthEncoding(t, nextT, hits.x, hits.y);
          t = nextT + epsilon;
        }
    ...
    color = accumColor / (hits.y - hits.x);
    color.w = 1.0;
    } else if (useCinematic || useLinear || useSpectral) {
      var maxVal = 0.0;
      while (t < hits.y) {
        let pt = p + d * t;
        let idx3D = floor((pt + 0.5) / voxelsize);
      ...
      if (useCinematic) {
        color = cinematicTF(maxVal);
      } else if (useLinear) {
        color = linearTF(maxVal);
      } else if (useSpectral) {
        color = spectralEdgeTF(maxVal);
      }
    } else  {
      //MIP
      ...
      while (t < hits.y) {
        ...
        if (idx >= 0 && idx < i32(arrayLength(&volData))) {
          let val = volData[idx];
          maxIntensity = max(maxIntensity, val);
        }
      ...
      let intensity = clamp(maxIntensity / 4095.0, 0.0, 1.0);
      color = vec4f(intensity, intensity, intensity, 1.0);
    }
    }
...
fn computeProjectiveMain(@builtin(global_invocation_id) global_id: vec3u) {
  ...
  if (uv.x < texDim.x && uv.y < texDim.y){
    ...
    var rdir = vec3f(px * cameraPose.focal.x, py * cameraPose.focal.y, 1.0);
    spt = transformPt(spt);
    rdir = transformDir(rdir);
    traceScene(uv, spt, rdir);
  }
